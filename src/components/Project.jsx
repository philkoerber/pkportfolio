...
function Project(props) {
  ...
  return (
    <ParallaxDiv
      className="relative w-fit max-w-[300px] h-fit font-urbanist"
      offset={100}>
      {/* PICTURE AND TITLE */}
      ...
      <div className="relative mx-auto w-[90%] md:w-full max-w-[300px]">
        ...
        <svg
          className="absolute top-[-52%] rotate-[89deg]"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 -100 200 300"
          fill="none">
          <text
            textAnchor="left"
            className="text-center font-instrument text-2xl tracking-widest"
            x="50%"
            y="0%"
            dy="-0em"
            fill="#000000">
            <textPath xlinkHref="#text-curve">{content.name}</textPath>
          </text>
          <path
            id="text-curve"
            d="M150,150a100,100 0 1,1 0,-200a100,100 0 1,1 0,200"
          />
        </svg>
      </div>
      ...
      <div className="relative mx-auto w-[80%] z-50 bg-opacity-95 p-3">
        ...
        <div className="flex flex-wrap gap-x-2 gap-y-0 justify-end">
          {content.techs.map((tech, i) => {
            return (
              <div key={i} className="text-delftblue w-fit">
                #{tech}
              </div>
            );
          })}
